...
public class confirmSettings extends Activity{
	...
	//BH!
	CheckBox checkBoxWater_bowl;
	CheckBox checkBoxBarrier;
	CheckBox checkBoxScreen;

	private static boolean currentWater_Bowl;
	private static boolean currentBarrier;
	...
	public void onActivityResult(int requestCode, int resultCode, Intent data) {//If back on phone pressed, retrieve data for intent
		...
		if (data==null)
		{
			BenignNum.setText(Integer.valueOf(this.bw.Benign).toString());
			ControlNum.setText(Integer.valueOf(this.bw.Control).toString());
			MalignantNum.setText(Integer.valueOf(this.bw.Malignant).toString());
			Log.e("Loading Activity", "nulls datas...");
		}
		else if (data.hasExtra("Benign")) {
			System.out.println(" Set wheel data in confirm setting");
			this.bw.setWheelData(data);
			BenignNum.setText(Integer.valueOf(this.bw.getBenign()).toString());
			ControlNum.setText(Integer.valueOf(this.bw.Control).toString());
			MalignantNum.setText(Integer.valueOf(this.bw.Malignant).toString());
		}
		if(data!=null) {
	...
	public void onConfirmButtonClick (View v) {
		...
		Intent i = new Intent(this, trialRunActNew.class);

		System.out.println("After Crashing");
		pushDataOnIntent(i);
	...
	private void setViews(){
		...
		//BH!
		//get Water Bowl from settings screen to confirm settings
		preferences = getSharedPreferences("edu.upenn.cis350.cancerDog.conditions", Context.MODE_PRIVATE);
		currentWater_Bowl =preferences.getBoolean("water_bowl", false);
		checkBoxWater_bowl.setChecked(currentWater_Bowl);

		//get Water Bowl from settings screen to confirm settings
		preferences = getSharedPreferences("edu.upenn.cis350.cancerDog.conditions", Context.MODE_PRIVATE);
		currentBarrier = preferences.getBoolean("barrier", false);
		checkBoxBarrier.setChecked(currentBarrier);

		//get Water Bowl from settings screen to confirm settings
		preferences = getSharedPreferences("edu.upenn.cis350.cancerDog.conditions", Context.MODE_PRIVATE);
		currentScreen = preferences.getBoolean("screen", false);
		checkBoxScreen.setChecked(currentScreen);
	...
	public int getBenign(){
		return this.bw.getBenign();
	}
	public int getMalignant(){
		return this.bw.getMalignant();
	}

	public int getControl(){
		return this.bw.getControl();
	}
